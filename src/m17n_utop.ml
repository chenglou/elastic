let () =
  if List.exists ((=) "camlp4o") !Topfind.predicates ||
    ...
    UTop.parse_toplevel_phrase := UTop.parse_default (
      M17n_util.internationalize ~load_path:UTop.load_path Myparser.toplevel_phrase);
    UTop.parse_use_file := UTop.parse_default (
      M17n_util.internationalize ~load_path:UTop.load_path Myparser.use_file);
    ...
    Toploop.parse_use_file := M17n_util.internationalize Myparser.use_file;
  end
