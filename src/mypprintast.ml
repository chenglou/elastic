...
class printer  ()= object(self:'self)
  ...
  method core_type f x =
    ...
    | Ptyp_arrow (l, ct1, ct2) ->
        let rec print_arrow f x = (match x with
          | {ptyp_desc=Ptyp_arrow (l, ct1, ct2);} ->
              pp f "@[@;%a%a@]"
                self#type_with_label (l,ct1)
                print_arrow ct2
          | _ -> pp f "@;%a" self#core_type x) in
        pp f "@[<2>(->%a)@]" (* FIXME remove parens later *)
          print_arrow x
    | Ptyp_alias (ct, s) ->
  ...
  method core_type1 f x =
    ...
    | Ptyp_tuple l ->  pp f "<@[<hov>%a@]>" (self#list self#core_type1 ~sep:"@;") l
    | Ptyp_constr (li, l) ->
      (match l with
        | [] -> self#longident_loc f li
        | _ -> pp f "(%a@;%a)"
          self#longident_loc li
          (self#list ~first:"" ~last:"" self#core_type ~sep:" ") l)
  ...
  method pattern1 (f:Format.formatter) (x:pattern) :unit =
         ...
         Ppat_construct
           ({ txt = Lident("::") ;_},
            ...
            match pat2 with
              ...
              | _ -> pp f "%a"  self#simple_pattern  pat1 (*RA*)
            end
    ...
    | Ppat_construct (({txt;_} as li), po) -> (* FIXME The third field always false *)
        if txt = Lident "::" then
          ...
          | None -> begin match li with
            | {txt=Lident "true"} | {txt=Lident "false"} -> pp f "%a" self#longident_loc li
            | _ -> pp f "(%a)" self#longident_loc li
          end )
  ...
  method simple_pattern (f:Format.formatter) (x:pattern) :unit =
    ...
    | Ppat_record (l, closed) ->
        ...
        | _ ->
            pp f "@[<2>{%a _}@]"
              (self#list longident_x_pattern ~sep:"@;") l)
    | Ppat_tuple l -> pp f "@[<1><@[<hov>%a@]>@]" (self#list  ~sep:"@;" self#pattern1)  l (* level1*)
  ...
  method expression f x =
    ...
    | Pexp_fun (l, e0, p, e) ->
        let rec pp_print_pexp_function f x =
          ...
          | _ -> pp f "]@;<1 2>@[<hov>%a@])" self#expression x in
        pp f "(fun [%a%a"
    ...
    | Pexp_match (e, l) ->
        pp f "@[<hv2>(match %a@\n%a)@]" self#reset#expression e self#case_list l
    | Pexp_try (e, l) ->
        pp f "@[<0>@[<hv2>try@ %a@]@ @[<0>with%a@]@]" (* "try@;@[<2>%a@]@\nwith@\n%a"*)
          self#reset#expression e  self#case_list l
    | Pexp_let (rf, l, e) ->
        let rec pp_print_let_binding f x =
          match x.pexp_desc with
          | Pexp_let (rf, l, e) ->
              (match l with
                | [x] -> (match e with
                  | {pexp_desc=Pexp_let (_, _, _)} -> pp f "@[<hov>%a@\n%a@]" self#reset#binding x pp_print_let_binding e
                  | _ -> pp f "%a%a" self#reset#binding x pp_print_let_binding e)
                | _ -> assert false) (* shouldn't happen in our lisp *)
          | _ -> () in
        let rec pp_print_body_expression f x =
          match x.pexp_desc with
          | Pexp_let (rf, l, e) -> pp_print_body_expression f e
          | _ -> pp f "%a" self#expression x in
        (* pp f "@[<2>let %a%a in@;<1 -2>%a@]" (\*no identation here, a new line*\) *)
        (*   self#rec_flag rf *)
        pp f "(let @[<hov>[%a]@]@;<1 2>%a)"
          pp_print_let_binding x
          pp_print_body_expression x
    | Pexp_apply (e, l) ->
      if not (self#sugar_expr f x) then
        begin match view_fixity_of_exp e with
        | `Infix s | `Prefix s ->
          let s = if List.mem s ["~+";"~-";"~+.";"~-."] then String.sub s 1 (String.length s -1) else s in
            (* pp f "@[<2>(%s@;%a)@]" s (self#list (pp_print_infix_to_prefix s)) l *)
            pp f "(%s @[<hov>%a@])" s (self#list ~sep:"@;" self#reset#label_x_expression_param) l
        | `Normal ->
          pp f "@[<hov2>%a@]"
          (fun f (e,l) ->
            pp f "(%a@;@[<hov2>%a)@]"
              self#expression2 e
              (self#list ~sep:"@;" self#reset#label_x_expression_param) l)

              (* if the first element is an identifier, we wrap the expression with parens,
                 otherwise we assume expression2 will do it for us *)
              (* | {pexp_desc= Pexp_ident _} | {pexp_desc= Pexp_fun _} -> pp f "(%a@;@[<hov>%a)@]" self#expression2 e
                          (self#list self#reset#label_x_expression_param)  l
              | _ ->  pp f "(%a@;@[<hov>%a)@]" self#expression2 e
                          (self#list self#reset#label_x_expression_param)  l
      ...
      (*(if not (self#sugar_expr f x) then
          ...
          | `Infix s | `Prefix s ->
            ...
            (match l with
            | [ arg1; arg2 ] ->
                pp f "@[<2>(%s %a@;%a)@]" (* FIXME associativity lable_x_expression_parm*)
                  s self#label_x_expression_param arg1 pp_print_infix_to_prefix (snd arg2)
            | _ -> pp f "@[<2>(%a %a)@]" self#simple_expr e  (self#list self#label_x_expression_param) l)
          | `Normal ->
            pp f "@[<hov2>%a@]"
            (fun f (e,l) ->
              match e with
                (* if the first element is an identifier, we wrap the expression with parens,
                   otherwise we assume expression2 will do it for us *)
                | {pexp_desc= Pexp_ident _} | {pexp_desc= Pexp_fun _} -> pp f "@[<hov>(%a@ %a)@]" self#expression2 e
                            (self#list self#reset#label_x_expression_param)  l
                | _ ->  pp f "%a%a" self#expression2 e
                            (self#list self#reset#label_x_expression_param)  l
                            (*reset here only because [function,match,try,sequence] are lower priority*))
            (e,l)) *)
    ...
    | Pexp_ifthenelse (e1, e2, eo) ->
        pp f "@[<2>(if %a@\n%a%a@]"
          self#under_ifthenelse#expression e1
          self#under_ifthenelse#expression e2
          (fun f eo -> match eo with
          | Some x -> pp f "@;@[<2>%a)@]" self#under_semi#expression  x
          | None -> pp f ")") eo

        (*let rec pprint f e = match e with
        | Some {pexp_desc=Pexp_ifthenelse (e1,e2,eo)} ->
          pp f "@\n%a %a%a"
            self#under_ifthenelse#expression e1
            self#under_ifthenelse#expression e2
            pprint eo
        | Some exp ->
          pp f "@\ntrue %a"
            self#under_ifthenelse#expression exp
        | None -> () in
        begin
          match eo with
          | Some {pexp_desc=Pexp_ifthenelse (_,_,_)} ->
            pp f "@[<2>(cond @\n%a@ %a%a)@]"
              self#under_ifthenelse#expression e1
              self#under_ifthenelse#expression e2
              pprint eo
  ...
  method simple_expr f x =
    ...
    | Pexp_construct _  when is_simple_construct (view_expr x) ->
        ...
        | `simple x -> begin match x with
          | Lident "true" | Lident "false" -> pp f "%a" self#longident x
          | _ -> pp f "(%a)" self#longident x
          end
        | _ -> assert false)
    | Pexp_ident li ->
        self#longident_loc f li
    ...
    | Pexp_tuple l ->
        pp f "@[<hov><%a>@]"  (self#list self#simple_expr  ~sep:"@;")  l
    ...
    | Pexp_record (l, eo) ->
        ...
        begin match eo with
        | None -> pp f "@[<hv0>@[<hv2>{%a@]}@]"(* "@[<hov2>{%a%a}@]" *)
            (self#list longident_x_expression ~sep:"@;")  l
        | Some some_eo -> pp f "@[<hv0>@[<hv2>(update@;%a@;{%a@]})@]"(* "@[<hov2>{%a%a}@]" *)
            self#simple_expr some_eo
            (self#list longident_x_expression ~sep:"@;")  l
        end
    | Pexp_array (l) ->
        pp f "@[<0>@[<2>[%a]@]@]"
  ...
  method binding f {pvb_pat=p; pvb_expr=x; _} =
    ...
    | Pexp_constraint (e,t1),Ppat_var {txt;_} ->
        pp f "%s@ (%a@;:@;%a)" txt self#expression e self#core_type t1
    | (_, Ppat_var _) ->
  ...
  method bindings f (rf,l) =
    let binding kwd rf f x = match rf with
      | Nonrecursive ->
        pp f "(%s %a)" kwd
          self#binding x
      | Recursive ->
  ...
  method structure_item f x = begin
    ...
    | Pstr_eval (e, attrs) -> self#expression f e
  ...
  method  type_def_list f l =
    let type_decl kwd f x =
      ...
      in *)
      begin match x.ptype_params with
      | [] -> pp f "@[<2>(%s %s%a)@]%a"
          kwd
          x.ptype_name.txt
  ...
  method type_declaration f x =
    ...
    let constructor_declaration f pcd =
      ...
      | None ->
          pp f "@[<2>(%s%a%a)@]" pcd.pcd_name.txt
             self#attributes pcd.pcd_attributes
             (fun f -> function
              | [] -> ()
              | l -> pp f "@;@[<hov>%a@]" (self#list self#core_type1 ~sep:"@;") l)
             pcd.pcd_args
      | Some x ->
    ...
    let repr f =
      ...
      | Ptype_record l ->
          pp f "%t@;{%a}" intro
             (self#list ~sep:"@;" label_declaration)  l ;
...
let string_of_expression x =
  ...
  flush_str_formatter () ;;
let string_of_structure ?size x =
  ignore (flush_str_formatter ());
  let f = str_formatter in
  (match size with
  | Some s -> Format.pp_set_margin f s
  | None -> ());
